...
def run_core_tests(repo: Path, config_path: Path, out: list[Check]):
    ...
    expired_header = api_headers(dyn_key,"GET",req_uri,body,int(BASE_NOW)-61,"dyn-expired")
...
def run_webui_test(repo: Path, config_path: Path, out: list[Check]):
    ...
    add(out,"webui","version is v4.3.11",None if "<span>v4.3.11</span>" in page[1] and "3.0.0" not in page[1] else 500,None,"WebUI brand version")
