...
| dynamic | strict mode with sign disabled | `403` | `403` | **PASS** | explicit configuration error; no Cookie fallback |
...
| webui | version is v4.3.0 | `ALLOW` | `ALLOW` | **PASS** | WebUI brand version |
...
| webui | config page exposes v4 policy fields | `ALLOW` | `ALLOW` | **PASS** | fields=['cfg-key-mode', 'cfg-dynamic-strict-sign', 'cfg-dynamic-sign-ratio-fail', 'cfg-dynamic-allow-cookie-fallback', 'cfg-strict-api-paths', 'cfg-dynamic-document-paths', 'cfg-dynamic-cookie-tag-hex', 'cfg-cookie-document-require-fetch'] |
| webui | config save enforces dynamic-only | `ALLOW` | `ALLOW` | **PASS** | static/legacy/secret inputs removed |
...
| performance | dynamic-only 300 signed requests | `ALLOW` | `ALLOW` | **PASS** | elapsed=0.0298s, rps=10081.7, denied=0 |

## 运行边界

这是本地 Lua/OpenResty 核心模拟，不会向生产发送请求。`ALLOW` 只代表 RFW 层放行，不代表业务授权成功。v4.3.0 dynamic-only 严格模式要求非文档请求携带当前 dynamic RFWDATA；默认 `dynamic_allow_cookie_fallback=false`，仅在管理员显式开启且已有有效 dynamic `_RFW` Cookie 时进入完整 Cookie 兼容例外。
