...
> **v4.3.12 当前修复**：`rfw_version` 与 `rfw_protocol` 都是发布兼容性边界；任一不兼容时，客户端 fail-closed 并提示刷新，避免旧缓存脚本继续解释更新后的服务端实现。此修正不改变请求/响应字段、签名格式或可恢复 403 语义。Status 首页继续显示长期累计“请求总数”；历史图的请求量改为每个日志日的实际增量。
